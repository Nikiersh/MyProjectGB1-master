...
bool SPaperZDAnimOutlinerItem::IsHovered() const
{
	TSharedPtr<FPaperZDAnimTimelineTrack> PinnedTrack = Track.Pin();
	if (PinnedTrack.IsValid())
	{
		return SMultiColumnTableRow<TSharedRef<FPaperZDAnimTimelineTrack>>::IsHovered() || PinnedTrack->IsHovered();
	}

	return SMultiColumnTableRow<TSharedRef<FPaperZDAnimTimelineTrack>>::IsHovered();
}
