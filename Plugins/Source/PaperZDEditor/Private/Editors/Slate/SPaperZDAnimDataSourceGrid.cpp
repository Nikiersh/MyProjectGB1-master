...
void SPaperZDAnimDataSourceGrid::Construct(const FArguments& InArgs)
{
	...
	BackgroundImage = FEditorStyle::GetBrush("Graph.Panel.SolidBackground");
	NodeBrush = FEditorStyle::GetBrush("CurveEd.CurveKey");
	DirectionalAreaBrush = FPaperZDEditorStyle::Get().GetBrush("PaperZDEditor.WhiteBrush");

	// Retrieve UI color values
	NodeColor = FEditorStyle::GetSlateColor("BlendSpaceKey.Regular");
	HighlightNodeColor = FEditorStyle::GetSlateColor("BlendSpaceKey.Highlight");
	SelectNodeColor = FEditorStyle::GetSlateColor("BlendSpaceKey.Pressed");
	PreviewNodeColor = FEditorStyle::GetSlateColor("BlendSpaceKey.Preview");
...
void SPaperZDAnimDataSourceGrid::PaintAreas(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32& DrawLayerId) const
{
	...
	if (CachedNodeLocations.IsValidIndex(SelectedNodeIndex))
	{
		...
		Verts.Add(FSlateVertex::Make<ESlateVertexRounding::Disabled>(SlateRenderTransform, AllottedGeometry.Size / 2.0f, FVector2D(0.0f, 1.0f), SelectedAreaColor)); //center
		Verts.Add(FSlateVertex::Make<ESlateVertexRounding::Disabled>(SlateRenderTransform, CachedNodeLocations[SelectedNodeIndex], FVector2D(0.0f, 1.0f), SelectedAreaColor)); //node
		Verts.Add(FSlateVertex::Make<ESlateVertexRounding::Disabled>(SlateRenderTransform, CachedMidPointLocations[SelectedNodeIndex], FVector2D(0.0f, 1.0f), SelectedAreaColor)); //next mid-point

		const int32 PrevMidPointIndex = (SelectedNodeIndex - 1 + CachedMidPointLocations.Num()) % CachedMidPointLocations.Num();
		Verts.Add(FSlateVertex::Make<ESlateVertexRounding::Disabled>(SlateRenderTransform, CachedMidPointLocations[PrevMidPointIndex], FVector2D(0.0f, 1.0f), SelectedAreaColor)); //prev mid-point
	...
	if (CachedNodeLocations.IsValidIndex(CursorArea))
	{
		...
		//Add the points: center, node, next mid-point, prev mid-point
		const FSlateRenderTransform& SlateRenderTransform = AllottedGeometry.ToPaintGeometry().GetAccumulatedRenderTransform();
		Verts.Add(FSlateVertex::Make<ESlateVertexRounding::Disabled>(SlateRenderTransform, AllottedGeometry.Size / 2.0f, FVector2D(0.0f, 1.0f), DrawColor)); //center
		Verts.Add(FSlateVertex::Make<ESlateVertexRounding::Disabled>(SlateRenderTransform, CachedNodeLocations[CursorArea], FVector2D(0.0f, 1.0f), DrawColor)); //node
		Verts.Add(FSlateVertex::Make<ESlateVertexRounding::Disabled>(SlateRenderTransform, CachedMidPointLocations[CursorArea], FVector2D(0.0f, 1.0f), DrawColor)); //next mid-point
		
		const int32 PrevMidPointIndex = (CursorArea - 1 + CachedMidPointLocations.Num()) % CachedMidPointLocations.Num();
		Verts.Add(FSlateVertex::Make<ESlateVertexRounding::Disabled>(SlateRenderTransform, CachedMidPointLocations[PrevMidPointIndex], FVector2D(0.0f, 1.0f), DrawColor)); //prev mid-point
...
void SPaperZDAnimDataSourceGrid::PaintNodes(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32& DrawLayerId) const
{
	...
	for (int32 i = 0; i < CachedNodeLocations.Num(); i++)
	{
		...
		FSlateDrawElement::MakeBox(OutDrawElements, DrawLayerId + 1, AllottedGeometry.ToPaintGeometry(GridPosition, NodeSize), NodeBrush, ESlateDrawEffect::None, DrawColor);
	}
	...
	//Paint the preview node
	{
		...
		FSlateDrawElement::MakeBox(OutDrawElements, DrawLayerId + 1, AllottedGeometry.ToPaintGeometry(GridPosition, NodeSize), NodeBrush, ESlateDrawEffect::None, PreviewNodeColor.GetSpecifiedColor());
	}
}
