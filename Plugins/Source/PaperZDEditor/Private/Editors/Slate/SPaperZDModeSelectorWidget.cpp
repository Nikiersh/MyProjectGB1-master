...
void SPaperZDModeSelectorWidget::OnCheckboxStateChanged(ECheckBoxState InState, EPaperZDModeSection Section)
{
	if (Section == EPaperZDModeSection::Blueprint)
	{
		...
		else
		{
			...
			Filter.ClassNames.Add(UPaperZDAnimBP::StaticClass()->GetFName());
			...
			{
				GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OpenEditorForAsset(AssetDataArray[0].ObjectPath.ToString());
			}
		}
	}
	else
...
void SPaperZDModeSelectorWidget::HandleAssetSelectedFromPicker(const FAssetData& SelectedAsset)
{
	...
	GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OpenEditorForAsset(SelectedAsset.ObjectPath.ToString());
}
